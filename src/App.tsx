        ...
        {activeTab === "usage" && (
          <section className="tab-panel active">
            <article className="card">
              ...
              {usage ? <UsageSection usage={usage} /> : <div className="detail-empty">No usage data.</div>}
            </article>
          </section>
        )}
      </main>
...
function TaskDetail({
  ...
  const importantLogs = selectImportantLogs(task.logs);
  const logsToRender = showAllLogs ? task.logs : importantLogs;
  const hiddenLogCount = Math.max(0, task.logs.length - importantLogs.length);
  ...
  return (
    <div className="detail-card">
      ...
      <div className="log-list">
        ...
        {task.logs.length > importantLogs.length ? (
          <button type="button" className="ghost log-toggle" onClick={() => setShowAllLogs((prev) => !prev)}>
            {showAllLogs ? "Show key logs only" : `Show all logs (${hiddenLogCount} hidden)`}
          </button>
        ) : null}
      </div>
    </div>
...
function UsageSection({ usage }: { usage: UsageOverview }) {
  const meter = extractUsageMeter(usage);

  return (
    <div className="usage-stack">
      <div className="usage-primary">
        <div className="meta">Used / Total Available</div>
        <div className="usage-primary-value">
          <span className="title">{formatNumber(meter.used)}</span>
          <span className="meta"> / {meter.total !== null ? formatNumber(meter.total) : "unknown"}</span>
        </div>
        <div className="meta">{meter.remaining !== null ? `Remaining: ${formatNumber(meter.remaining)}` : "Remaining: unknown"}</div>
        <div className="usage-progress" role="img" aria-label={meter.ratio !== null ? `Usage ${Math.round(meter.ratio * 100)} percent` : "Usage ratio unavailable"}>
          <div className="usage-progress-bar" style={{ width: `${meter.ratio !== null ? Math.max(0, Math.min(100, meter.ratio * 100)) : 0}%` }} />
        </div>
        <div className="meta">
          {meter.ratio !== null
            ? `Current usage ratio: ${(meter.ratio * 100).toFixed(1)}%`
            : "Ratio unavailable because total available usage is missing from API response."}
        </div>
      </div>
...
function extractUsageMeter(usage: UsageOverview): { used: number; total: number | null; remaining: number | null; ratio: number | null } {
  const used = pickNumber(usage, [
    "used",
    "usedTokens",
    "tokenUsed",
    "tokensUsed",
  ...
  const remaining = pickNumber(usage, ["remaining", "remainingTokens", "tokenRemaining", "tokensRemaining", "available", "left"]);
  const ratioFromPayload = pickNumber(usage, ["ratio", "usageRatio", "usedRatio", "usagePercent", "usedPercent"]);

  const computedUsed = used ?? (total !== null && remaining !== null ? total - remaining : null);
  const computedTotal = total ?? (used !== null && remaining !== null ? used + remaining : null);
  const computedRemaining = remaining ?? (computedTotal !== null && computedUsed !== null ? computedTotal - computedUsed : null);

  if (computedUsed === null) {
    return { used: 0, total: computedTotal, remaining: computedRemaining, ratio: normalizeRatio(ratioFromPayload) };
  }

  if (computedTotal === null || computedTotal <= 0) {
    return { used: computedUsed, total: computedTotal, remaining: computedRemaining, ratio: normalizeRatio(ratioFromPayload) };
  }

  return { used: computedUsed, total: computedTotal, remaining: computedRemaining, ratio: computedUsed / computedTotal };
}
...
function selectImportantLogs(logs: TaskLog[]) {
  const important = logs.filter((entry) => isImportantLog(entry));
  if (important.length > 0) {
    return important.slice(-10);
  }
  return logs.slice(-3);
}

function isImportantLog(entry: TaskLog) {
  const text = entry.message.toLowerCase();
  const level = typeof entry.level === "string" ? entry.level.toLowerCase() : "";
  const kind = typeof entry.type === "string" ? entry.type.toLowerCase() : "";

  if (["error", "warn", "fatal", "critical", "success"].includes(level)) {
    return true;
  }
  if (["error", "warning", "approval", "status", "summary"].includes(kind)) {
    return true;
  }
  ...
  const noisePatterns = ["heartbeat", "poll", "debug", "trace", "stdout", "stderr", "keepalive", "health check", "fetching"];

  if (importantPatterns.some((pattern) => text.includes(pattern))) {
    return true;
  }
